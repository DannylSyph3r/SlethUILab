...
const colorPresets: ColorPreset[] = [
  { name: "Neon", colorDark: "#0a0515", colorMid: "#581c87", colorBright: "#ec4899", colorAccent: "#06b6d4" },
  { name: "Flame", colorDark: "#0c0a09", colorMid: "#7c2d12", colorBright: "#f97316", colorAccent: "#fbbf24" },
  { name: "Ocean", colorDark: "#020617", colorMid: "#1e3a8a", colorBright: "#3b82f6", colorAccent: "#67e8f9" },
  { name: "Gold", colorDark: "#0f0c06", colorMid: "#78350f", colorBright: "#f59e0b", colorAccent: "#fef3c7" },
  { name: "Aurora", colorDark: "#022c22", colorMid: "#065f46", colorBright: "#10b981", colorAccent: "#a78bfa" },
  { name: "Violet", colorDark: "#1a0a1a", colorMid: "#86198f", colorBright: "#d946ef", colorAccent: "#f0abfc" },
  { name: "Sunset", colorDark: "#1a0a0f", colorMid: "#9d174d", colorBright: "#fb7185", colorAccent: "#fcd34d" },
  { name: "Forest", colorDark: "#0a1a0f", colorMid: "#166534", colorBright: "#22c55e", colorAccent: "#86efac" },
  { name: "Midnight", colorDark: "#030712", colorMid: "#1e1b4b", colorBright: "#6366f1", colorAccent: "#c4b5fd" },
  { name: "Glacier", colorDark: "#0c1929", colorMid: "#155e75", colorBright: "#22d3ee", colorAccent: "#ecfeff" },
  { name: "Rose", colorDark: "#1a0a10", colorMid: "#881337", colorBright: "#f43f5e", colorAccent: "#fda4af" },
  { name: "Ember", colorDark: "#1c0a05", colorMid: "#9a3412", colorBright: "#ea580c", colorAccent: "#fed7aa" },
];
            ...
            <div className="p-4 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10">
              {activeSection === "flow" && (
                <div className="space-y-5">
                  <ControlRow label="Wave Complexity" value={settings.waveComplexity} min={0.3} max={2.0} step={0.1} onChange={(v) => onSettingsChange({ waveComplexity: v })} />
                  <ControlRow label="Flow Warp" value={settings.flowIntensity} min={0} max={2.0} step={0.1} onChange={(v) => onSettingsChange({ flowIntensity: v })} />
                  <ControlRow label="Pattern Scale" value={settings.noiseScale} min={0.3} max={2.0} step={0.1} onChange={(v) => onSettingsChange({ noiseScale: v })} />
                  <ControlRow label="Flow Speed" value={settings.animationSpeed} min={0.02} max={0.3} step={0.02} onChange={(v) => onSettingsChange({ animationSpeed: v })} />
                  <ControlRow label="Dark Valleys" value={settings.contrastBoost} min={0.5} max={1.5} step={0.05} onChange={(v) => onSettingsChange({ contrastBoost: v })} />
                </div>
              )}

              {activeSection === "glass" && (
                <div className="space-y-5">
                  <ControlRow label="Ridge Depth" value={settings.displacementStrength} min={0.02} max={0.4} step={0.01} onChange={(v) => onSettingsChange({ displacementStrength: v })} />
                  <ControlRow label="Ridge Count" value={settings.lineFrequency} min={20} max={200} step={5} onChange={(v) => onSettingsChange({ lineFrequency: v })} formatValue={(v) => v.toFixed(0)} />
                  <ControlRow label="Ridge Waviness" value={settings.ridgeWaviness} min={0} max={1.5} step={0.05} onChange={(v) => onSettingsChange({ ridgeWaviness: v })} />
                  <ControlRow label="Film Grain" value={settings.grainIntensity} min={0} max={0.12} step={0.005} onChange={(v) => onSettingsChange({ grainIntensity: v })} formatValue={(v) => v.toFixed(3)} />
                </div>
              )}
              ...
              {activeSection === "color" && (
                <div className="space-y-5">
                  <div
                    ...
                    style={{ background: `linear-gradient(90deg, ${settings.colorDark}, ${settings.colorMid}, ${settings.colorBright}, ${settings.colorAccent})` }}
                  />
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {[
                      ...
                      <div key={key} className="space-y-1.5">
                        <span className="text-[9px] text-white/30 uppercase tracking-wider block text-center">{label}</span>
                  ...
                  <div className="pt-2">
                    <span className="text-[9px] text-white/30 uppercase tracking-wider block mb-3">Presets</span>
                      ...
                      {colorPresets.map((preset) => (
                        ...
                        >
                          <div
                            ...
                            style={{ background: `conic-gradient(from 0deg, ${preset.colorDark}, ${preset.colorMid}, ${preset.colorBright}, ${preset.colorAccent}, ${preset.colorDark})` }}
                          />
